...
Connects to VPS server at 148.251.88.48
"""
...
if getattr(sys, "frozen", False):
    ...
    system_site_packages = os.path.expanduser(
        "~/Library/Python/3.9/lib/python/site-packages"
    )
    if (
        os.path.exists(system_site_packages)
        and str(system_site_packages) not in sys.path
    ):
...
def setup_ui_files():
    ...
    if getattr(sys, "frozen", False):
        ...
        if not index_html_path.exists() or index_html_path.stat().st_size == 0:
            ...
            if source_index.exists() and source_index.stat().st_size > 0:
                ...
                print(
                    f"✓ Restored index.html from source ({source_index.stat().st_size} bytes)"
                )
            else:
    ...
    if app_js_path.exists() and app_js_path.stat().st_size > 0:
        ...
        content = content.replace(
            "const API_BASE = '/api';", f"const API_BASE = '{VPS_API_BASE}';"
        )
    ...
    if index_html_path.exists() and index_html_path.stat().st_size > 0:
        ...
        if f"http://localhost:{LOCAL_PORT}/static/" not in content:
            ...
            content = content.replace(
                "/static/", f"http://localhost:{LOCAL_PORT}/static/"
            )
        # Also ensure app.js script tag uses the correct URL
        content = re.sub(
            r'src=["\']http://localhost:\d+/static/app\.js["\']',
            f'src="http://localhost:{LOCAL_PORT}/static/app.js"',
            content,
        )
        content = re.sub(
            r'src=["\']/static/app\.js["\']',
        ...
        print(
            f"⚠ Warning: index.html not found or empty (exists: {index_html_path.exists()}, size: {index_html_path.stat().st_size if index_html_path.exists() else 0})"
        )
